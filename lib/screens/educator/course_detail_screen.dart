...
class _CourseDetailScreenState extends State<CourseDetailScreen>
  ...
  Future<void> _createAttendanceSession() async {
    ...
    try {
      // Display time picker for selecting session time range
      final timeRange = await _showSessionTimePicker();

      if (timeRange == null) return; // User canceled the time picker
      ...
      if (session != null) {
        ...
        // Should not reach here with the rethrow in the service
        ScaffoldMessenger.of(context).showSnackBar(
          const SnackBar(
            content: Text('Failed to open attendance session'),
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Error opening attendance session: $e'),
          backgroundColor: Colors.red,
        ),
      );
  ...
  Future<void> _toggleSessionStatus(AttendanceSession session) async {
      ...
      SnackBar(
        content: Row(
          children: [
            ...
            Text(
                session.isOpen ? 'Closing session...' : 'Reopening session...'),
          ],
        ),
    ...
    try {
      if (kDebugMode) {
        print(
            'Toggling session ${session.id} from isOpen=${session.isOpen} to isOpen=${!session.isOpen}');
      }
      ...
      if (updatedSession != null) {
        ...
        _loadSessions(); // Reload sessions to update the UI
      }
    } catch (e) {
      // Dismiss the loading indicator
      if (mounted) {
  ...
  // Display time picker for selecting session time range
  Future<Map<String, DateTime>?> _showSessionTimePicker() async {
    ...
    // Use course defaults if available
    if (widget.course.defaultStartTime != null) {
      startTime = DateTime(
        now.year,
        now.month,
        now.day,
        widget.course.defaultStartTime!.hour,
        widget.course.defaultStartTime!.minute,
      );
    }

    if (widget.course.defaultEndTime != null) {
      endTime = DateTime(
        now.year,
        now.month,
        now.day,
        widget.course.defaultEndTime!.hour,
        widget.course.defaultEndTime!.minute,
      );
    }

    // Show dialog for time selection
    final result = await showDialog<Map<String, DateTime>>(
      context: context,
      builder: (context) {
        // Create stateful local variables for time selection
              ...
              content: Column(
                ...
                children: [
                  ListTile(
                    ...
                    subtitle: Text(DateFormat('h:mm a').format(localStartTime)),
                  ...
                  ListTile(
                    ...
                    subtitle: Text(DateFormat('h:mm a').format(localEndTime)),
                    trailing: const Icon(Icons.access_time),
                    onTap: () async {
                      final TimeOfDay? picked = await showTimePicker(
                        context: context,
                        initialTime: TimeOfDay.fromDateTime(localEndTime),
                      );
                  ...
                  Text(
                    'Duration: ${localEndTime.difference(localStartTime).inMinutes} minutes',
                      ...
                      child: ExpansionTile(
                        ...
                        trailing: ElevatedButton(
                          style: ElevatedButton.styleFrom(
                            backgroundColor:
                                session.isOpen ? Colors.red : Colors.green,
                          ),
                          onPressed: () => _toggleSessionStatus(session),
                          child: Text(
                            session.isOpen ? 'Close' : 'Reopen',
                    ...
                    return Card(
                      ...
                      child: ListTile(
                        ...
                        trailing: IconButton(
                          ...
                          onPressed: () {
                            _showStudentAttendanceHistory(student.toJson());
                          },
                        ),
                      ),
                    );
