    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        if (error['detail'] is List) {
          // Handle case where the API returns a list of error messages
          final errorList = error['detail'] as List;
          errorMessage = errorList.isNotEmpty
              ? errorList.first.toString()
              : 'Failed to check in';
        } else {
          // Handle case where API returns a string error message
          errorMessage = error['detail']?.toString() ?? 'Failed to check in';
        }
